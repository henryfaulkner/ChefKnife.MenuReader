...
(function (global, factory) {
  ...
  function movable($stage, $image) {
    ...
    var dragStart = function dragStart(e) {
      ...
      // zxc is the difference between image width and height
      zxc = !_this.isRotated ? 0 : (imageWidth - imageHeight) / 2;
      ...
      left = $image.position().left - zxc;
      top = $image.position().top + zxc;
    ...
    var dragMove = function dragMove(e) {
      ...
      // Vertical limit
      if (heightDiff > 0) {
        if (relativeY + top > zxc) {
          newTop = zxc;
        } else if (relativeY + top < -heightDiff + zxc) {
          newTop = -heightDiff + zxc;
        }
      } else {
        newTop = top;
      }
      // Horizontal limit
      if (widthDiff > 0) {
        if (relativeX + left > -zxc) {
          newLeft = -zxc;
        } else if (relativeX + left < -widthDiff - zxc) {
          newLeft = -widthDiff - zxc;
        }
      } else {
        newLeft = left;
      }
  ...
  function resizable($modal, $stage, $image, options) {
    ...
    var getImageCSS = function getImageCSS(dir, offsetX, offsetY) {
      ...
      var widthDiff = offsetX + modalData.w > minModalWidth ? stageData.w - imgFrameWidth + offsetX - zxc : minModalWidth - (modalData.w - stageData.w) - imgFrameWidth - zxc;
      var heightDiff = offsetY + modalData.h > minModalHeight ? stageData.h - imgFrameHeight + offsetY + zxc : minModalHeight - (modalData.h - stageData.h) - imgFrameHeight + zxc;
      var widthDiff2 = -offsetX + modalData.w > minModalWidth ? stageData.w - imgFrameWidth - offsetX - zxc : minModalWidth - (modalData.w - stageData.w) - imgFrameWidth - zxc;
      var heightDiff2 = -offsetY + modalData.h > minModalHeight ? stageData.h - imgFrameHeight - offsetY + zxc : minModalHeight - (modalData.h - stageData.h) - imgFrameHeight + zxc;

      // Get the image position on resizing
      var $imageLeft = $image.position().left;
      var $imageTop = $image.position().top;
      var imgLeft = (widthDiff > 0 ? $imageLeft : Math.min($imageLeft, 0)) - zxc;
      var imgTop = (heightDiff > 0 ? $imageTop : Math.min($imageTop, 0)) + zxc;
      var imgLeft2 = (widthDiff2 > 0 ? $imageLeft : Math.min($imageLeft, 0)) - zxc;
      var imgTop2 = (heightDiff2 > 0 ? $imageTop : Math.min($imageTop, 0)) + zxc;
      var dirsCSS = {
        e: {
          left: widthDiff >= -zxc ? (widthDiff - zxc) / 2 : Math.max(imgLeft, widthDiff)
        },
        s: {
          top: heightDiff >= zxc ? (heightDiff + zxc) / 2 : Math.max(imgTop, heightDiff)
        },
        se: {
          top: heightDiff >= zxc ? (heightDiff + zxc) / 2 : Math.max(imgTop, heightDiff),
          left: widthDiff >= -zxc ? (widthDiff - zxc) / 2 : Math.max(imgLeft, widthDiff)
        ...
        w: {
          left: widthDiff2 >= -zxc ? (widthDiff2 - zxc) / 2 : Math.max(imgLeft2, widthDiff2)
        },
        n: {
          top: heightDiff2 >= zxc ? (heightDiff2 + zxc) / 2 : Math.max(imgTop2, heightDiff2)
        },
        nw: {
          top: heightDiff2 >= zxc ? (heightDiff2 + zxc) / 2 : Math.max(imgTop2, heightDiff2),
          left: widthDiff2 >= -zxc ? (widthDiff2 - zxc) / 2 : Math.max(imgLeft2, widthDiff2)
    ...
    var dragStart = function dragStart(dir, e) {
      ...
      // zxc is the difference between image width and height
      zxc = !_this.isRotated ? 0 : (imageData.w - imageData.h) / 2;
  ...
  var PhotoViewer = /*#__PURE__*/function () {
    ...
    _createClass(PhotoViewer, [{
      ...
      value: function zoomTo(ratio, origin) {
        ...
        // zxc is the difference between new width and new height of the image
        var zxc = !this.isRotated ? 0 : (newWidth - newHeight) / 2;
        ...
        if (imgFrameWidth <= stageWidth) {
          ...
          newLeft = newLeft > -zxc ? -zxc : Math.max(newLeft, diffX + zxc);
        }
        if (imgFrameHeight <= stageHeight) {
          newTop = diffY / 2;
        } else {
          newTop = newTop > zxc ? zxc : Math.max(newTop, diffY - zxc);
        }
